...
{
	// Central hub — command consoles and equipment
	...
	// West barracks — wall-mounted terminals
	SpawnConsole(FVector(-81500.f, -3000.f, 10.f), 90.f);
	SpawnConsole(FVector(-78500.f, 3000.f, 10.f), -90.f);

	// Shelving / crate stacks in buildings
	FLinearColor CrateColor(0.08f, 0.07f, 0.06f);
	struct FInteriorCrate { FVector Pos; FVector Scale; float Yaw; };
	TArray<FInteriorCrate> Crates = {
		// Hub interior storage
	...
	{
		SpawnStaticMesh(C.Pos, C.Scale, FRotator(0.f, C.Yaw, 0.f),
			CubeMesh, CrateColor);
	}
}
