	...
	float RotSpeed, float PropScale)
{
	...
	// Try real mesh for base pillar
	UStaticMesh* RealBase = LoadObject<UStaticMesh>(nullptr,
		TEXT("/Game/Meshes/Quaternius_SciFi/Column_Slim"));
	if (RealBase) BaseMesh->SetStaticMesh(RealBase);

	UMaterialInterface* LitMat = FExoMaterialFactory::GetLitEmissive();
	if (!LitMat) return;

	bool bHasRealBase = (RealBase != nullptr);
	FLinearColor DarkMetal(0.05f, 0.05f, 0.07f);

	// Base pillar — only override material if using primitive mesh
	if (!bHasRealBase)
	{
	...
	{
		UStaticMesh* RealVent = LoadObject<UStaticMesh>(nullptr,
			TEXT("/Game/Meshes/Quaternius_SciFi/Details/Details_Vent_1"));
		...
		if (RealVent)
		{
			SpinningPart->SetStaticMesh(RealVent);
			SpinningPart->SetRelativeScale3D(FVector(1.5f) * PropScale);
		}
		else
		{
